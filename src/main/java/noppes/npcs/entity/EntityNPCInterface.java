        ...
        if (this.ai.onAttack == 1)
            ...
            if (this.inventory.getProjectile() == null || this.ai.useRangeMelee == 2) {
                switch (this.ai.tacticalVariant) {
                    case Dodge:
                        this.tasks.addTask(this.taskCount++, aiResponse = new EntityAIZigZagTarget(this, 1.2D, this.ai.tacticalRadius));
                        break;
                    case Surround:
                        this.tasks.addTask(this.taskCount++, aiResponse = new EntityAIOrbitTarget(this, 1.2D, this.ai.tacticalRadius, true));
                        break;
                    case HitNRun:
                        this.tasks.addTask(this.taskCount++, aiResponse = new EntityAIAvoidTarget(this));
                        break;
        ...
        if (this.display.collidesWith != 1) {
            ...
            if (list != null && !list.isEmpty()) {
                for (int i = 0; i < list.size(); ++i) {
                    ...
                    if (this.canBePushed() || (entity instanceof EntityNPCInterface && (this.display.collidesWith == 2 || this.display.collidesWith == 4)) || (entity instanceof EntityPlayerMP && (this.display.collidesWith == 3 || this.display.collidesWith == 4))) {
